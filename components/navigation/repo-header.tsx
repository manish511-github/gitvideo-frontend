...
export function RepoHeader({
  ...
  // Get repository name based on ID
  const getRepoName = () => {
    if (selectedRepoId === 1) return "Product Demo v2.0"
    if (selectedRepoId === 2) return "Company Overview"
    if (selectedRepoId === 3) return "Tutorial Series"
    if (selectedRepoId === 4) return "Marketing Campaign"
    return "Repositories"
  }
      ...
      <div className="flex h-10 items-center">
        ...
        <div className="flex items-center" style={{ width: "272px" }}>
          <div className="flex items-center h-full border-r w-full px-3">
            {onBack && selectedRepoId ? (
              <Button variant="ghost" size="icon" onClick={onBack} className="h-6 w-6 mr-2">
            ...
            <span className="text-xs font-medium mr-8 truncate max-w-[100px]">{getRepoName()}</span>
